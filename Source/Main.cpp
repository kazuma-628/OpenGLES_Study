...
//	シェーダーファイルの読み込みを行う
//
void Shader_FileLoad(const char* shader_name, char* shader_source, int source_size)
{

	FILE *fp;		//ファイルポインタ宣言
	char String_Line[SHADER_STRING_LINE_MAX] = { 0 };		//1行の読み込み最大数

	char shader_file_name[SHADER_FILE_NAME_MAX] = "..\\Shader\\";

	strcat_s(shader_file_name, shader_name);

	//ファイルのオープン
	assert(0 == fopen_s(&fp, shader_file_name, "r"));
